    ...
    public List<IVideoPage> parse(String pageUri, int pageNo) throws IOException, ParserException, URISyntaxException {
        ...
        while (iter.hasMoreNodes()) {
            ...
            if (video != null) {
                ...
                // parse image
                ImageTag img = (ImageTag) HtmlParserUtils.getTag(itemHtml, ARDMediathekParser.CHARSET, "div.mt-image img");
                if (img != null) {
                    try {
                        video.setThumbnail(new URI(ARDMediathekParser.BASE_URL + img.getImageURL()));
